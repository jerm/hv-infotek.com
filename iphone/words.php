		...
		if ($count_bp != 0){
			for ($x = 0;$x<count($a1);$x++){
						...
						if (!($x == count($a1)-1 && $y == count($a2)-1 && $z == count($a3)-1)){ 
							...
							$final_string = $a1[$x] . $a2[$y] . $a3[$z];
							$count_array = str_split($final_string);
							$letter_count = 0;
							for ($c=0;$c<count($count_array);$c++){
								if (ctype_alpha($count_array[$c])){
									$letter_count++;
								}
							}
							//print $letter_count . " " . $final_string . "<br>";
							if($letter_count > 2){
								mysql_query("insert into word_assoc values ('" . $n .  "','" . $a1[$x] . $a2[$y] . $a3[$z] . "',$letter_count)"); 
							}
						} 
        				}
	      			}
...
$fr = mysql_query("select distinct * from word_assoc where number = $n order by letters desc");
//print "<hr>";
